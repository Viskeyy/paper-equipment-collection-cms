...
export const SanitaryNapkinADiapersTable = ({ equipmentType }: { equipmentType: string }) => {
    ...
    return (
        <Card title={isDiapers ? '纸尿裤检测数据列表' : '卫生巾检测数据列表'} className="w-full">
            <Table
                title={() => (
                    <Row>
                        <Col span={24}>
                            <Button
                                type="primary"
                                onClick={() =>
                                    navigate({ to: isDiapers ? '/diapers/create' : '/sanitary-napkin/create' })
                                }
                            >
                                新增记录
                            </Button>
